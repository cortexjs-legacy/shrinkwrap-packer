...
function defaultFilter(mod) {
  return function(src) {
    ...
    if (ext == ".js" || ext == ".css") {
      return !/\.min\.|\.__origin__\./.test(src);
    } else {
      return true;
    }
  }
...
Packer.prototype.build = function(callback) {
  ...
  var mod = this.mods[0];
  // ignore multi mods;
  var isFullVersion = _.isString(mod.version);
...
Packer.prototype.fullVersionFiles = function(mod, done) {
  ...
  this._getShrinked(mod, function(err, json) {
    if (err) {
      return done(err);
    }
    var arr = [];
...
Packer.prototype._getShrinked = function(mod, done) {
  var moddir = path.join(this.root, mod.name, mod.version);
  var shrinkwrap_path = path.join(moddir, "cortex-shrinkwrap.json");
  fs.exists(shrinkwrap_path,function(exists){
    if(!exists){
      return done("Shrinkwrap of " + mod.name + "@" + mod.version + " Not Found")
    }
    fs.readFile(shrinkwrap_path, function(err, content){
      if(err){return done(err);}
      var json = JSON.parse(content);
      json.version = mod.version;
      done(null, shrinked.parse(json,{
...
Packer.prototype.extend = function(mod, done, filter) {
  ...
  this._getShrinked(mod, function(err, tree) {
    if (err) {
      return done(err);
    }
    var result = {};
